...
def resize_and_zip_images(src_image_bytes: bytes, src_image_name: str, sizes: list[str]) -> bytes:
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zipf:
        for size_str in sizes:
            try:
                ...
                file_name = f"{name}-{size}{src_path.suffix}"
...
def body():
    uploaded_file = st.file_uploader("Upload an icon", type=["png", "jpg", "jpeg", "webp"])
    ...
    if uploaded_file and sizes_str:
        ...
        with st.spinner("Resizing images..."):
            zip_bytes = resize_and_zip_images(uploaded_file.getvalue(), uploaded_file.name, sizes)
            if zip_bytes:
                divider()
                st.download_button(
                    label="Download Resized Icons (.zip)",
                    data=zip_bytes,
                    file_name="resized-icons.zip",
